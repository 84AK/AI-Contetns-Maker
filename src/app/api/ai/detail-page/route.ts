...
- ctaText 입력값이 있으면 button 필드에 반드시 사용`;
        ...
        await Promise.all([
            logUsage(profile.id, "detail-page", "generate"),
            saveGeneratedContent({ userId: profile.id, type: "detail-page", title: data.hookHeadline ?? productName, productName, content: data, promptText }),
        ]);
